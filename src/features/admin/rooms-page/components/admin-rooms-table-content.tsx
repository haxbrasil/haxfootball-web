...
export function AdminRoomsTableContent({ rooms }: { rooms: Room[] }) {
      ...
      columns={[
        {
          ...
          cell: (room) =>
            room.program?.title
              ? localizedTextLabel(room.program.title)
              : (room.program?.name ?? room.id),
        },
        {
          key: "state",
          title: "Estado",
          ...
          cell: (room) => (
            <RoomActions
              ...
              onOpenDetails={(roomId) =>
                navigate({ to: "/admin/rooms/$roomId", params: { roomId } })
              }
            />
          ),
        },
